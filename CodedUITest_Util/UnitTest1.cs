...
{
    [TestClass]
    public class DESUtils_Test
    {
        [TestMethod]
        public void Test_DES()
        {
            string args = "abc一二三123";
            string jiami = Util.Cryptography.DESUtils.Encrypt(args);
            string r = Util.Cryptography.DESUtils.Decrypt(jiami);

            Assert.AreEqual<string>(args, r);
        }


        [TestMethod]
        public void Test_DES_MoreTextLen()
        {
...
巴拿马运河是连通太平洋和大西洋的重要国际水道，1914年正式开通。1999年，巴拿马收回运营主权。2016年，运河扩建工程完工。";
            ...
            string miwen = Util.Cryptography.DESUtils.Encrypt(args);
            string r = Util.Cryptography.DESUtils.Decrypt(miwen);

            Assert.AreEqual<string>(args, r);            
        }

        [TestMethod]
        public void Test_DES_StringEmpty()
        {
