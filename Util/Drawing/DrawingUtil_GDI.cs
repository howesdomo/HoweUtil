...
namespace Util.Drawing
{
    ...
    public static class DrawingUtil_GDI
    {
        ...
        #region 提供一个思路, 界面中预览的处理

        //// 界面中使用滚动条 或者 使用输入栏调整效果数据值, 
        //// 每次数值的更都要从原图中重新加载效果， 若每次都重新读取磁盘的文件效率将会大大降低,
        //// 故先复制原图在内存中的值到一个新的内存位置中
        //// 待下次更改数据值时, 从新位置拷贝数值到原来位置
        ...
        //public static void Copy(this Bitmap Bmp)
        //{
        //    IntPtr ImageCopyPointer = IntPtr.Zero;
        ...
        //                    Bmp.PixelFormat,
        //                    BmpData);    // 用原始格式LockBits,得到图像在内存中真正地址，这个地址在图像的大小，色深等未发生变化时，每次Lock返回的Scan0值都是相同的。
        ...
        //private static void UpdateImage(Bitmap Bmp)
        //{
        //    IntPtr ImageCopyPointer = IntPtr.Zero;
        //    IntPtr ImagePointer = IntPtr.Zero;
        //    int DataLength = 0;
